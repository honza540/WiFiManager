...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .clean-form { display: flex; justify-content: flex-end; margin: 0; }
...
<body>
  <main>
    ...
    <section>
      ...
      <div class="block">
        ...
        <div class="table-wrap">
          <table aria-label="Saved WiFi credentials">
            <thead>
              <tr><th>Index</th><th>Saved SSID</th><th>Signal now</th><th>Action</th></tr>
            </thead>
            <tbody id="credentialRows">
              <tr class="empty-row"><td colspan="4">Loading saved slots</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div class="block">
        ...
        <dl>
          <dt>BT device</dt><dd id="btDevice">-</dd>
          <dt>Pairing password</dt><dd id="btPassword">-</dd>
        </dl>
      </div>
    </section>
  </main>
  ...
  <script>
    ...
    const previewSetup = {
      ...
      bt: { device_name: 'BT-BeSmarter', password: '0000' },
      credentials: [
    ...
    function renderCredentials() {
      ...
      for (let idx = 0; idx < 3; idx += 1) {
        ...
        button.disabled = !credential.valid;
    ...
    async function loadSetup() {
      try {
        ...
        setText('btDevice', data.bt && data.bt.device_name);
        setText('btPassword', data.bt && data.bt.password);
        renderCredentials();
      } catch (error) {
    ...
    async function cleanCredential(form) {
      const idxInput = form.querySelector('input[name="idx"]');
      const idx = idxInput ? idxInput.value : '';

      if (location.protocol === 'file:') {
        setup.credentials = setup.credentials.map(item =>
          String(item.idx) === String(idx) ? { idx: Number(idx), valid: false, ssid: '' } : item
        );
        renderCredentials();
        return;
      }
    ...
    document.addEventListener('submit', async event => {
      ...
      if (form.action.indexOf('/clear') >= 0) {
        ...
        try {
          await cleanCredential(form);
        } catch (error) {
          scanState.textContent = 'Clean unavailable';
          if (button) {
      ...
      if (form.action.indexOf('/save') < 0) return;
